...
13. Every domain has at least one service — never create an empty domain
14. Domain `services` frontmatter lists every service that belongs to that domain
15. Every domain has a `ubiquitous-language.mdx` file with relevant domain terms extracted from services, events, commands, and business processes
