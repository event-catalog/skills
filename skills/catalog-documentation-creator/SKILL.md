...
6. Message relationships (`sends`/`receives`) include `id`
7. Channel routing (`to`/`from`) references valid channel IDs
8. Body includes `<NodeGraph />` for visualization
9. Schema references point to real files
10. Folder structure follows catalog conventions
11. No duplicate resources (checked against existing catalog)
12. File is named `index.mdx` (not `index.md`, `README.md`, or anything else)
13. Every domain has at least one service or agent — never create an empty domain
14. Domain `services` and `agents` frontmatter lists every service and agent that belongs to that domain
15. Every domain has a `ubiquitous-language.mdx` file with relevant domain terms extracted from services, agents, events, commands, and business processes
